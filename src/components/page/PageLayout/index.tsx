...
const PageLayout: FC<PageLayoutProps> = ({ children, footer }) => {
  ...
  const childClassName = cn("w-[300px] sm:w-[450px] lg:w-[700px] opacity-0 transition-opacity", selectedTool ? "opacity-100 animate-[fadeIn_1s_ease_forwards]" : "")

  const footerClassName = cn(
    "w-full fixed top-1/2 bottom-1/2 flex flex-col items-center justify-center",
    selectedTool ? "top-[unset] bottom-0 justify-center items-center animate-slidedown" : ""
  );

  return (
    <div className="h-full relative flex justify-center">
      {selectedTool && <div className={childClassName}>{children}</div>}
